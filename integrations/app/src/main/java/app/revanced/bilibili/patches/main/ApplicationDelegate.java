...
public abstract class ApplicationDelegate extends Application {
    ...
    static {
        try {
            System.loadLibrary("biliroamingzq");
        } catch (Throwable t) {
            Logger.error(t, () -> "Failed to load biliroamingzq library");
        }
    ...
    public void onCreate() {
        ...
        Logger.debug(() -> String.format("Initializing BiliRoamingZQ on process %s cost %s ms", Utils.currentProcessName(), end - start));
    }
